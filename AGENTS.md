...
Use this map to decide which files must be inspected before implementation.

### Lead import tasks

Inspect:

- `backend/app/schemas/lead.py`# LeadOS Agent Operating Instructions
...
- Do not mark a task done if required validation is failing or unverified. If local validation is blocked by missing tools, GitHub Backend CI may serve as the completion source of truth after push.
