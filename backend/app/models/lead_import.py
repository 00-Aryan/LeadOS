...
class LeadImportRun(Base):
    ...
    errors = relationship("LeadImportError", back_populates="import_run", cascade="all, delete-orphan")
...
class LeadImportError(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    import_run_id: Mapped[int] = mapped_column(ForeignKey("lead_import_runs.id"), nullable=False, index=True)
